function [varargout] = CoordG2L( Rg2l, Vg2l, varargin )
    ...
    if length(varargin) == 1 % P_global has format of [nframes x 3 x N markers]
        ...
        nargoutchk(1, 1)
        varargout{1} = permute(pagemtimes(permute(P_global - repmat(Vg2l, 1, 1, Nm), [4 2 1 3]), repmat(nRg2l, 1, 1, 1, Nm)), [3 2 4 1]);
    ...
    elseif length(varargin) > 1 % P_global of each marker are input separately.
        ...
        for k = 1:nVarargs
            varargout{k} = permute(pagemtimes(permute(varargin{k} - Vg2l, [3 2 1]), nRg2l), [3 2 1]);
