...
base_path = r'C:\Users\gib445\surfdrive - Larsen, A.G. (Aske Gye)@surfdrive.surf.nl\PHD Aske\Paper Ideas\OMC vs IMU vs Video'

# Append subfolders to the base path
pathToDataRelativeAngles = os.path.join(base_path, 'Angles_suit')
pathToDataEvents = os.path.join(base_path, 'Events_QTM')
...
if 'pathToDataRelativeAngles' in locals():
    for file1 in os.listdir(pathToDataRelativeAngles):
        ...
        group.append(int(''.join(filter(str.isdigit, file1))))

group = np.array(group)

# Extract data from the loaded .mat files
for numtrials in range(len(filename)):
    data[numtrials] = data[numtrials]['Angles_Suit']  # Adjust key if needed
...
refactor_value = original_frequency_s / frequency  # Adjust based on your file's original frequency
...
for numtrials in range(len(filenameE)):
    dataE[numtrials] = dataE[numtrials]['Events_QTM']  # Adjust the key if needed
...
for indx in range(len(dataE)):
    ...
    original_frequency = original_frequency_s  # Adjust this to the correct frequency of your data
...
np.save(os.path.join(pathToCleanData, f"stored_3D_other_data_IMU_latentfeatures1_{latentFeatures}_frequency_{frequency}"), other_data)
np.save(os.path.join(pathToCleanData, f"stored_y_3D_adapted_IMU_latentfeatures1_{latentFeatures}_frequency_{frequency}"), y_adapted)
np.save(os.path.join(pathToCleanData, f"stored_3D_groupsplit_IMU_latentfeatures1_{latentFeatures}_frequency_{frequency}"), groupsplit)

filename = 'exclude_files_IMU.txt'
